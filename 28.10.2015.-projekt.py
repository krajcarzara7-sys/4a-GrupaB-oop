...
class EvidencijaApp:
    ...
    def kreiraj_gui(self):
        ...
        unos = tk.Frame(self.root, padx=10, pady=10, bg='purple')
        ...
        tk.Button(gumbi, text="Dodaj učenika", bg='white', command=self.dodaj_ucenika).pack(side="left", padx=4)

        
        tk.Button(gumbi, text="Spremi CSV", bg='yellow', command=self.spremi_u_csv).pack(side="left", padx=4)
        tk.Button(gumbi, text="Učitaj CSV", bg='pink', command=self.ucitaj_iz_csv).pack(side="left", padx=4)

        
        tk.Button(gumbi, text="Spremi XML", bg='green', command=self.spremi_u_xml).pack(side="left", padx=4)
        tk.Button(gumbi, text="Učitaj XML", bg='light blue', command=self.ucitaj_iz_xml).pack(side="left", padx=4)
...
#1. RAM se briše svaki put kad se računalo isključi ili izgubi napajanje. Koristimo ga za privremenu i brzu trajnu podataka, a trajnu memoriju za dugoročno čuvanje i ponovnu upotrebu.
#2. CSV pohranjuje podatke u tabličnom obliku (redovi i stupci su odvojeni razmacima) i jednostavan je i lak za čitanje,ali ne opisuje značenje podataka i odnose među njima. XML koristi oznake za opis podataka i njihove hijerarhijske odnose, složeniji je, ali sam opisuje strukturu i značenje podataka pomoću oznaka.
#3. Bolja je od ručnog zatvaranja jer i u slučaju pojavljivanja greške datoteka će se sigurno zatvoriti.
